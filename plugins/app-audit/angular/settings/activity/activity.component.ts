...
@Component({selector: 'app-settings-activity', templateUrl: './activity.component.html'})
...
export class SettingsActivityComponent implements OnInit {
  ...
  plugin = {'id': 17, 'name': 'Application Audit'};

  // Object Definitions (Non-Configurable)
  objectName = 'Activity';
  objectDefinition = 'Activity';
  ...
  routeAllowed() {
    ...
    if (!rightLib.sufficientPluginActionRights(this.data.userData.pluginActionRight, this.plugin.id, 'view', 'all')) { this.router.navigate([`dashboard`]); }
